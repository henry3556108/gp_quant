#!/usr/bin/env python3
"""
Compare soft_min vs mean aggregation methods

Usage:
    python scripts/compare_softmin_vs_mean.py --plot boxplot
    python scripts/compare_softmin_vs_mean.py --plot pnl
"""
...
def plot_boxplot(results: Dict[str, List[Dict]], bh_train: float, bh_test: float):
    ...
    fig.suptitle('soft_min vs mean 聚合方法比較\n(8:2 Split, pop=1000)', 
                 fontsize=14, fontweight='bold', y=1.02)
    
    plt.tight_layout()
    
    output_path = BASE_DIR / "softmin_vs_mean_with_ted_niche_boxplot.png"
...
# =============================================================================
# Step 7b: Plot PnL curves (separate figures for RSFGP and Rolling)
# =============================================================================
def plot_pnl_curves(results: Dict[str, List[Dict]], bh_train_pnl: pd.Series, bh_test_pnl: pd.Series):
    """
    Create PnL curve comparison - two separate figures:
    1. RSFGP figure with soft_min (top) and mean (bottom) subplots
    2. Rolling Window figure with soft_min (top) and mean (bottom) subplots
    
    Each subplot shows Train (top row) and Test (bottom row) with all runs.
    """
    # Color palette for 10 runs
    ...
    figures_config = [
        {
            'title': 'RSFGP: soft_min vs mean PnL 曲線比較',
            'filename': 'rsfgp_pnl_comparison.png',
            'groups': [("RSFGP\n(soft_min)", "soft_min"), ("RSFGP\n(mean)", "mean")]
        },
        {
            'title': 'Rolling Window: soft_min vs mean PnL 曲線比較',
            'filename': 'rolling_pnl_comparison.png',
            'groups': [("Rolling\n(soft_min)", "soft_min"), ("Rolling\n(mean)", "mean")]
        }
    ]
    ...
    for fig_config in figures_config:
        ...
        for row_idx, (group_key, method_label) in enumerate(fig_config['groups']):
        ...
        for row_idx, (_, method_label) in enumerate(fig_config['groups']):
            axes[row_idx, 0].annotate(
                method_label.upper(), xy=(-0.15, 0.5), xycoords='axes fraction',
                fontsize=12, fontweight='bold', rotation=90, va='center', ha='center'
            )
        
        fig.suptitle(fig_config['title'], fontsize=14, fontweight='bold', y=0.98)
        plt.tight_layout(rect=[0.02, 0, 1, 0.96])
        
        output_path = BASE_DIR / fig_config['filename']
        plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
        print(f"\n✅ {fig_config['filename']} 已保存至: {output_path}")
