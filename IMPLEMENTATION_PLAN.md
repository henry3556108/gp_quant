...
**版本**: 1.4  
**日期**: 2025-10-13  
**狀態**: Phase 1 完成，Phase 2.0 完成，Phase 2.1 完成，Phase 2.1.5 (Early Stopping) 完成  
**預估時程**: 4-6 週
...
- [ ] 組合評估的策略在測試集上表現優於單股票策略（更穩健 ex: sharpe ratio）
...
### Phase 2: Tree Similarity 計算模組 🔄 進行中

**分支**: `feature/tree-similarity`（或直接在 master）  
**預估時間**: 1.5-2 週  
**當前狀態**: 尚未開始，需要先補充 `Norm` operator

#### 2.0 補充缺失的 GP Operator (Week 3) 🎯 當前任務

**任務清單**:
- [ ] 實作 `Norm(r1, r2)` operator
  - [ ] 在 `gp_quant/gp/primitives.py` 中實作函數
  - [ ] 計算兩個數值向量差值的絕對值：`|r1 - r2|`
  - [ ] 處理向量化操作
  - [ ] 添加錯誤保護機制
- [ ] 註冊到 primitive set
  - [ ] 在 `gp_quant/gp/operators.py` 中註冊
  - [ ] 定義類型簽名：`[NumVector, NumVector] -> NumVector`
- [ ] 測試新 operator
  - [ ] 單元測試
  - [ ] 整合測試（確保 GP tree 可以使用）
...
#### 2.1 Tree Edit Distance 實作 (Week 3-4) ⏳ 待開始
...
**交付物**:
- `gp_quant/similarity/tree_edit_distance.py`（新增目錄）
- 演算法選擇文檔
- 單元測試
...
- [ ] TED 計算正確
- [ ] 處理各種樹結構（高度、複雜度）
- [ ] 性能可接受（< 100ms per pair）

#### 2.2 並行相似度計算 ⭐ 核心 (Week 4-5) ⏳ 待開始

**任務清單**:
- [ ] 實作 `ParallelSimilarityCalculator` 類別
  - [ ] 使用 multiprocessing.Pool (8 workers)
  - [ ] 將配對分配到各 worker
  - [ ] 收集並合併結果
- [ ] 實作 `SimilarityMatrix` 類別
  - [ ] 批次計算相似度
  - [ ] 標準化為 [0, 1]
  - [ ] 對稱矩陣優化（只計算上三角）
- [ ] 實作快取機制
...
### Phase 3: Niching 策略實作 ⏳ 待開始

**分支**: `feature/niching-strategy`（或直接在 master）  
**預估時間**: 2-2.5 週  
**前置條件**: Phase 2 完成
...
**任務清單**:
- [ ] 實作 `NichingClusterer` 類別
  - [ ] K-means clustering
  - [ ] Hierarchical clustering
  - [ ] 動態群數決定（Elbow method, Silhouette）
- [ ] 整合 Tree Similarity Matrix
- [ ] 聚類結果管理
  - [ ] 追蹤每個個體的 niche ID
  - [ ] 計算 niche 統計資訊
...
#### 3.2 跨群 Parent Selection (Week 1-2)

**任務清單**:
- [ ] 實作 `CrossNicheSelector` 類別
  - [ ] 修改 DEAP selection operator
  - [ ] 跨群選擇邏輯
  - [ ] 可配置跨群比例
- [ ] 整合到 `EvolutionEngine`
  - [ ] 在 selection 階段調用
